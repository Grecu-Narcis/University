...
namespace Lab4
{
    internal class TaskSolution
    {
        ...
        public TaskSolution(List<string> urls)
        {
            this.urls = urls;
        }
        ...
        public void Run()
        {
            ...
            var tasks = this.urls.Select(url => HandleUrl(url));

            Task.WhenAll(tasks).Wait();
        }

        private Task HandleUrl(string url)
        {
            ...
            Task<Socket> connectTask = ConnectSocket(socket, endPoint);
            connectTask.ContinueWith(prevTask => SendRequest(socket, urlPath, baseUrl))
                .ContinueWith(prevTask => ReceiveResponse(socket, receiveDone));
        ...
        private Task<Socket> ReceiveResponse(Socket socket, ManualResetEvent receiveDone)
        {
